 ...
 * @property {number} lifetime  // Added lifetime property to gridcell obj
...
function createCell(x, y) {
  ...
  let lifetime = 0;   //  If cell is alive initally, set lifetime as 1. Default is 0.
  if (alive) {
    lifetime = 1;
  }
  return {
...
export function createGame(context, numRows, numColumns) {
  ...
  function drawCell(cell) {
    ...
    let opacity = 0;                        // Opacity is calculated as multitudes of 0.25
    if (cell.lifetime > 3) {                // The maximum value possible is 1
      opacity = 1;
    } else {
  ...
  function updateGrid() {
    ...
    forEachCell((cell) => {
      ...
      if (numAlive === 2) {
        ...
        cell.lifetime++;        // Only changes remaining living cell
      } else if (numAlive === 3) {
        // Dead cell becomes living, living cell remains living
        cell.nextAlive = true;
        cell.lifetime++;        // Living cell lifetime is incremented by 1
        if (!cell.alive) {      // Dead cell lifetime is set to 1
          cell.lifetime = 1;
        }
      } else {
        // Living cell dies, dead cell remains dead
        cell.nextAlive = false;
        cell.lifetime = 0;      // Dead cell lifetime is set to 0
      }
    });
